...
if($showDis['status'] == 'Loan-Disbursal'){
    echo '<meta http-equiv="refresh" content="0;url=loan-disbursal.php?agent_id='.$agentID.'" />';
}
...
<div class="col-md-10 offset-md-1">
    <div class="card">
        <div class="card-body">
           <h4 class="font-width-bold font-italic text-center">Loan Approve Bank Feedback</h4>
        </div>
    </div>
    <br>
  ...
  <div class="row">
   ...
   <div class="col-md-8">
    ...
    if(isset($_POST['fdf'])){
        ...
        if(mysqli_query($conn, "INSERT INTO loan_ap_msg (loan_ap_msg, user_id, agent_id, who)VALUES('$fdm', '$userID', '$agentID', 'customer' )")){
  ...
  if(isset($_POST['FDFU'])){
      ...
      if($_FILES['further_doc_file']['size'] > 1048576){
       echo '<script>alert("Your File Is big! Please send 1 MB File")</script>';
          ...
          $upLodSucc = mysqli_query($conn, "INSERT INTO `loan_ap_doc_file` (`id`, `loan_ap_doc_file`, `user_id`, `agent_id`) VALUES (NULL, '$fileName', '$userID', '$agentID')");
          if($upLodSucc == true){
              move_uploaded_file($_FILES['further_doc_file']['tmp_name'], '../docFile/'.$fileName);
              echo '<meta http-equiv="refresh" content="0">';
          }else{
